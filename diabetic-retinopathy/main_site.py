...
def train_site(args, train_loader, eval_loader_dict, net, prev_model, round_num,best_net,best_performance,epoch):
    ...
    for j in range(args.rounds):     
        for iteration, data in enumerate(eval_loader_dict['val_loader' + str(j+1)]):
            inputs = data['image'].cuda()
            labels = data['label'].cuda()
...
for k in range(args.epochs_per):
    net,best_net,best_performance = train_site(args, train_loader_dict['train_loader' + str(k%args.rounds+1)], val_loader_dict, net, prev_model, k,best_net,best_performance,k)
    prev_model = copy.deepcopy(net)
