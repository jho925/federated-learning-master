...
def train_site(args, train_loader, eval_loader, ewc, round_num,best_ewc,best_performance,epoch,prev_model):
...
best_performance = 0  
ewc.model.train(True)
print("Starting Training Round on Set " + str(1) + "...")

for i in range(args.rounds):
    ewc,best_net,best_performace = train_site(args, train_loader_dict['train_loader' + str(i%args.rounds + 1)], val_loader_dict['val_loader' + str(i%args.rounds + 1)], ewc, i,best_net,best_performance,i,prev_model)
    ewc.register_ewc_params(train_loader_dict['train_loader' + str(i%args.rounds + 1)].dataset,args.batch_size)
    prev_model = copy.deepcopy(ewc.model)

for k in range(args.epochs_per):
    ewc,best_net,best_performace = train_site(args, train_loader_dict['train_loader' + str(k%args.rounds + 1)], val_loader_dict['val_loader' + str(k%args.rounds + 1)], ewc, k,best_net,best_performance,k,prev_model)
    prev_model = copy.deepcopy(ewc.model)
