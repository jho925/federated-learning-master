...
def train_site(args, train_loader, eval_loader_dict, net, prev_model, round_num,best_net,best_error,epoch):
...
for j in range(args.rounds):
    ...
    for k in range(args.epochs_per):
        net,best_net,best_error = train_site(args, train_loader_dict['train_loader' + str(args.sites*j+k%args.sites+1)], val_loader_dict, net, prev_model, j,best_net,best_error,k)
